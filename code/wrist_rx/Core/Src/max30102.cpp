...
bool MAX30105::begin(I2C_HandleTypeDef* wirePort, TIM_HandleTypeDef* ptimer, uint32_t i2cSpeed, uint8_t i2caddr) {
  ...
  // timer = ptimer; // Removed
  // HAL_TIM_Base_Start(timer); // Removed
...
extern "C" void MAX30102_Init_C(void *i2c_handle) {
    I2C_HandleTypeDef *hi2c = (I2C_HandleTypeDef*)i2c_handle;
    // Pass NULL for timer since we use HAL_GetTick
    if (particleSensor.begin(hi2c, NULL, I2C_STANDARD_RATE, 0x57)) {
         particleSensor.setup();
    }
}

extern "C" uint32_t MAX30102_GetIR_C(void) {
    return particleSensor.getIR();
}

extern "C" void MAX30102_Reset_C(void) {
